  ...
  <div class="space-y-4 w-full">
    ...
    <Teleport to="body">
      ...
      >
        ...
        <p v-if="tooltip.day.total_cost !== undefined" class="text-[11px] text-muted-foreground">
...
const recalcCellSize = () => {
  ...
  // 自适应尺寸，最小 6px，最大 10px（更小的方块）
  cellSize.value = Math.max(6, Math.min(10, rawSize))
}
...
function buildTooltip(day: ActivityHeatmapDay): string {
  ...
  const parts = [`${dateLabel}`, `${day.requests} 次请求`, `${formatTokens(day.total_tokens)} tokens`]
  if (day.total_cost !== undefined) {
    parts.push(formatCurrency(day.total_cost))
  }
