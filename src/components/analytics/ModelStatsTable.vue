<template>
  <CardSection class="flex flex-col h-full">
    <div class="mb-4 flex items-center justify-between">
      <h3 class="text-lg font-serif font-medium text-foreground">模型使用统计</h3>
    </div>
      ...
      <table class="w-full text-sm text-left">
        <thead class="text-xs text-muted-foreground uppercase border-b border-border/50">
          <tr>
            ...
            <th class="py-3 px-2 text-right font-medium">耗时</th>
            <th class="py-3 pl-2 text-right font-medium">成功率</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-border/30">
          <tr v-for="item in data" :key="`${item.provider}:${item.model}`" class="group hover:bg-muted/30">
            <td class="py-3 pr-4 font-medium text-foreground group-hover:text-primary transition-colors">
              {{ item.model }}
            </td>
            ...
            <td class="py-3 pl-2 text-right font-mono text-muted-foreground">
